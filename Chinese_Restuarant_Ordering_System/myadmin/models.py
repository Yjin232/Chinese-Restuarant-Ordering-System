...
class User(models.Model):
    ...
    status = models.IntegerField(default=1)    #状态:1正常/2禁用/3管理员/9删除
...
class Shop(models.Model):
    ...
    cover_pic = models.CharField(max_length=255) #封面图片路径
    banner_pic = models.CharField(max_length=255) #图标logo路径
    ...
    def toDict(self):
        ...
        return {'id':self.id,'name':shopname[0],'shop':shopname[1],
                'cover_pic':self.cover_pic,'banner_pic':self.banner_pic,
...
class Product(models.Model):
    ...
    name = models.CharField(max_length=255) # 菜品名称
    cover_pic = models.CharField(max_length=255) #菜品图片路径
    price = models.FloatField() #菜品价格信息
    status = models.IntegerField(default=1)#状态:1正常/2暂停/9删除
    create_at = models.DateTimeField(default=datetime.now) #创建时间
    update_at = models.DateTimeField(default=datetime.now) #修改时间
    ...
    def toDict(self):
        return {'id': self.id, 'shop_id': self.shop_id, 'category_id': self.category_id,
                'name': self.name, 'cover_pic': self.cover_pic, 'status': self.status, 'price': self.price,
                'create_at': self.create_at.strftime('%Y-%m-%d %H:%M:%S'),
                'update_at': self.update_at.strftime('%Y-%m-%d %H:%M:%S')}
...
class Orders(models.Model):
    ...
    user_id = models.IntegerField()    # 管理员id
    money = models.FloatField() #订单价格信息
    status = models.IntegerField(default=1)        #状态:1过行中/2无效/3已完成
    payment_status = models.IntegerField(default=1)  #状态:1未支付/2已支付/3已退款
    create_at = models.DateTimeField(default=datetime.now)    #创建时间
    update_at = models.DateTimeField(default=datetime.now)    #修改时间
